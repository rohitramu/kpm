...
func GetTemplateInput(kpmHomeDir string, packageFullName string, parentTemplate *template.Template, parameters *types.GenericMap) (*types.GenericMap, error) {
	...
	inputParameters, err = GetPackageParameters(template_package.GetDefaultParametersFile(packageDir))
	if err != nil {
		return nil, err
	}
...
func GetSharedTemplate(packageDir string) (*template.Template, error) {
	...
	var helpersDir = template_package.GetHelpersDir(packageDir)
...
func GetPackageInfo(kpmHomeDir string, packageDir string) (*types.PackageInfo, error) {
	...
	var packageInfoFile = template_package.GetPackageInfoFile(packageDir)
	...
	var interfaceFilePath = template_package.GetInterfaceFile(packageDir)
	...
	var parametersFile = template_package.GetDefaultParametersFile(packageDir)
	...
	var templatesDir = template_package.GetTemplatesDir(packageDir)
	...
	var dependenciesDir = template_package.GetDependenciesDir(packageDir)
...
func GetExecutableTemplates(parentTemplate *template.Template, packageDir string) ([]*template.Template, error) {
	...
	var executableTemplatesDir = template_package.GetTemplatesDir(packageDir)
...
func getValuesFromInterface(parentTemplate *template.Template, packageDir string, parameters *types.GenericMap) (*types.GenericMap, error) {
	...
	var interfaceFile = template_package.GetInterfaceFile(packageDir)
...
func GetPackageFullNamesFromLocalRepository(kpmHomeDir string) ([]string, error) {
	...
	var packageRepositoryDir = template_package.GetPackageRepositoryDir(kpmHomeDir)
	...
	for currentPathObj, ok := toVisit.Pop(); ok; currentPathObj, ok = toVisit.Pop() {
		...
		if err == nil {
			...
			var packageFullName = template_package.GetPackageFullName(packageInfo.Name, packageInfo.Version)
